...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("switches desktop packaging product names to nightly for nightly builds", () => {
    assert.equal(resolveDesktopProductName("0.0.17"), "T3 Code Pair (Alpha)");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "T3 Code Pair (Nightly)");
  });
  ...
  it.effect("never points any build at an update feed", () =>
    Effect.gen(function* () {
      for (const version of ["0.0.33-pr.8182.1", "0.0.33", "0.0.41-preview.20260912.1589"]) {
        ...
        assert.isNull(config.publish, version);
      }
    }).pipe(
      Effect.provide(
        ConfigProvider.layer(
  ...
  it.effect("applies platform-specific packaging to the build config", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(mac.dmg, {
        title: "T3 Code Pair (Alpha) 1.2.3 Installer",
      ...
      // entry advertises MimeType=x-scheme-handler/t3code-pair; for OAuth deep links.
      assert.deepStrictEqual((linux.linux as Record<string, unknown>).protocols, [
        { name: "T3 Code Pair", schemes: ["t3code-pair", "t3code-pair-dev"] },
      ]);
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    ...
    assert.include(entitlements, "<string>ABC1234567.com.alexonufrak.t3code.pair</string>");
  ...
  it.effect("adds passkey entitlements and both renderer protocols to signed macOS builds", () =>
    Effect.gen(function* () {
      ...
      assert.equal(config.appId, "com.alexonufrak.t3code.pair");
