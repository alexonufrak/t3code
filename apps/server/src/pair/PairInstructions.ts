...
- An assignment starts from the room checkout's branch (or the baseRef the Lead names) and merges back into it. The Lead approves it with pair_review. It is merged only when the user says so: from the room controls, or by telling the Lead, who then calls pair_integrate in that same turn, quoting them; the room merges and closes the card. Never merge an assignment branch with git yourself, and never merge one nobody asked for.
