...
const make = Effect.gen(function* () {
  ...
  return PairToolkit.of({
    ...
    pair_checkout: (input) =>
      caller.pipe(Effect.flatMap((threadId) => coordinator.checkout(threadId, input))),
    ...
    pair_integrate: (input) =>
      caller.pipe(Effect.flatMap((threadId) => coordinator.integrate(threadId, input))),
